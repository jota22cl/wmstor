...
use App\Filament\Resources\GuiaSalidaResource\Pages;
use App\Filament\Resources\GuiaSalidaResource\RelationManagers;
//use Filament\Forms\Components\Repeater;
use Icetalker\FilamentTableRepeater\Forms\Components\TableRepeater;
//use Filament\Forms\Components\Tabs;


class GuiaSalidaResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Guía de Salida';
    protected static ?string $navigationGroup = 'Gestión Bodega';
    protected static ?string $title = 'Guía de Salida';
    protected static ?int $navigationSort = 3;

    public static function form(Form $form): Form
    {
        $isCreating = request()->routeIs('filament.admin.resources.guia-salidas.create');
            ...
            ->schema([
                ...
                Wizard::make([
                    Wizard\Step::make('Encabezado')
                        //->description('Guia Salida: '.$folio)
                        ->description('Guía Salida: ' . session('vssFolio'))
                        //->description(fn() => 'Guía Salida: ' . ($form->getState()['numeroGuia'] ?? $folio))
                        ...
                        ->schema([
                            ...
                            Hidden::make('guia')->default('s'), //'i'=Guia de SALIDA
...
//                                ->columnSpan(2),
                                    ...
                                    //->schema([
                                        ...
                                        Select::make('producto_codigo')
                                            ...
                                            ->afterStateUpdated(function (callable $set, callable $get, $state) {
                                                // Verificar duplicidad por código
                                                //dd("GuiaSalidaResource",$state, collect($get('guiadetalles')));    //  $state ==== id del producto digitado
                                                $productosSeleccionados = collect($get('guiadetalles'))
                                                    ->pluck('producto_codigo')
                                                    ->filter();
                                                //dd($productosSeleccionados);
                                                if ($productosSeleccionados->contains($state)) {
                                                    $set('producto_codigo', null); // Resetea el valor si ya existe
                                                    Notification::make()
                                                        ->title('Producto duplicado')
                                                        ->body('Este código de producto ya ha sido seleccionado en el detalle.')
                                                        ->warning()
                                                        ->send();
                                                } else {
                                                    ...
                                                    if ($producto) {
                                                        ...
                                                        $unimedSalida = Unimedida::find($producto->unimed_salida_id);
                                                        $set('producto_unimed', $unimedSalida?->codigo ?? 'N/A');
                                                        $set('saldoDisponible', ($producto->saldoInicial + $producto->totalEntradas - $producto->totalSalidas - $producto->reservado));
                                                    }
                                                }
                                            })
                                            ...
                                            //        $set('producto_id', $producto->id);
                                            //        $unimedSalida = Unimedida::find($producto->unimed_salida_id);
                                            //        $set('producto_unimed', $unimedSalida?->codigo ?? 'N/A'); // Asigna el código o 'N/A' si no existe
                                            //        $set('saldoDisponible', ($producto->saldoInicial + $producto->totalEntradas - $producto->totalSalidas - $producto->reservado));
                                            //    }
                                            //})
                                            ->columnSpan(3),
                                        ...
                                        Select::make('producto_descripcion')
                                            ...
                                            ->reactive()
                                            ->afterStateUpdated(function (callable $set, callable $get, $state) {
                                                // Verificar duplicidad por descripción
                                                $productosSeleccionados = collect($get('guiadetalles'))
                                                    ->pluck('producto_descripcion')
                                                    ->filter();
                                
                                                if ($productosSeleccionados->contains($state)) {
                                                    $set('producto_descripcion', null); // Resetea el valor si ya existe
                                                    Notification::make()
                                                        ->title('Producto duplicado')
                                                        ->body('Esta descripción de producto ya ha sido seleccionada en el detalle.')
                                                        ->warning()
                                                        ->send();
                                                } else {
                                                    $producto = Producto::find($state);
                                        ...
                                        Hidden::make('saldoDisponible'),
                                        TextInput::make('cantidad')
                                            ->label('Cantidad')
                                            ->required()
                                            ->numeric(8,2)
                                            ->minValue(1)
                                            ->maxValue(function (callable $get) { return $get('saldoDisponible') ?? 0; })
                                            ->placeholder(function (callable $get) { return '' . $get('saldoDisponible'); })
    ...
    {
        //dd(auth()->user()->empresa->folioGuiaSalida);
        // Ejemplo básico de generación de folio: incrementar el último valor
        //$lastFolio = Programa::max('folio');
        $newFolio = auth()->user()->empresa->folioGuiaSalida + 1;

        $empresa = auth()->user()->empresa;
        $empresa->folioGuiaSalida = $newFolio;
        $empresa->save();
            ...
            ->columns([
                ...
                TextColumn::make('emiteNumeroGuia')
                    ...
                    ->getStateUsing(fn ($record) => 
                        $record->estado === 'd'
                            ? '<a href="' . route('guia-salidas.GuiaSalida', $record->id) . '" 
                                target="_blank" 
                                style="
                                    display: inline-flex; 
                                    align-items: center; 
    ...
    {
        return [
            'index' => Pages\ListGuiaSalidas::route('/'),
            'create' => Pages\CreateGuiaSalida::route('/create'),
            'edit' => Pages\EditGuiaSalida::route('/{record}/edit'),
            'view' => Pages\ViewGuiaSalida::route('/{record}'),
        ];
    }
