...
use App\Mail\GuiaIngresoMailable;
...
class GuiaIngresoController extends Controller
{
    ...
    public function GuiaIngreso($id)
    {
        ...
        $GuiaIngreso = Guia::with(['guiadetalles','contrato','contrato.ccosto','contrato.bodega'])->findOrFail($id);
        //dd($GuiaIngreso);
        // Generar el PDF usando la vista y pasando los datos necesarios
        $pdf_view = PDF::setPaper('letter')->loadView('pdf.GuiaIngreso', compact('GuiaIngreso'));
        $pdf_save = PDF::setPaper('letter')->loadView('pdf.GuiaIngreso', compact('GuiaIngreso'));
        $origcp = $GuiaIngreso->estado == 'd' ? 'Original' : 'Copia';
        $nombrepdf = 'GuiaIngreso-'.$GuiaIngreso->numeroGuia.'-'.$origcp;
        ...
        DB::transaction(function () use ($pdf_save, $path, $GuiaIngreso) {
            if($GuiaIngreso->estado == 'd'){
                //cambia el etado de Digitado a Emitido en la guia.
                $GuiaIngreso->estado = 'e';
                $GuiaIngreso->fechaEmision = now();
                $GuiaIngreso->save();
                foreach ($GuiaIngreso->guiadetalles as $detalle) {
                    ...
                    DB::table('productos')
                        ...
                        ->increment('totalEntradas', $detalle->cantidad); // Incrementar el campo entradas
                ...
                $destinatarios = ContratoGuiaMail::where('contrato_id',$GuiaIngreso->contrato_id)
                    ->pluck('email')
                    ->toArray();
                //dd($destinatarios);
                Mail::to($destinatarios)
                   ->send(new GuiaIngresoMailable($path, $GuiaIngreso));
            }
        });
